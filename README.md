...
As we mentioned in the introduction, The Runtime RabbitMQ wrapper is not meant to be used by its own. Using it in the development of components for the RT-Constellation can be achieved also by configuring the appropriate dependency in the `pyproject.toml` of the components as:
```toml
